...
export default function TourViewer360({ scene, scenes, onSelectScene }: TourViewer360Props) {
  ...
  const mockHotspots = [
    { targetId: "scene-pool", name: "To Infinity Horizon Pool", angle: 25, label: "Horizon Pool Portal" },
    { targetId: "scene-vault", name: "To Private Reserve Vault", angle: 75, label: "Wine Cellar Portal" },
    { targetId: "scene-spa", name: "To Zenith Wellness Spa", angle: 10, label: "Thermal Bath Access" },
    { targetId: "scene-atelier", name: "To Grand Atelier Suites", angle: 50, label: "Master Wing" }
  ...
  return (
    <div
      ...
      ref={containerRef}
      className={`relative w-full overflow-hidden bg-black transition-all duration-500 rounded-lg ${
    ...
    >
            ...
            return (
              ...
              >
                ...
                <div className="relative flex items-center justify-center">
                  ...
                  {/* Hover tooltip */}
                  <div className="absolute bottom-12 left-1/2 -translate-x-1/2 bg-black/85 backdrop-blur-md border border-outline-lucid/30 py-2 px-3 rounded shadow-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 w-max pointer-events-none">
                    <p className="font-label-sm text-[10px] text-primary-fixed uppercase tracking-wider">{sceneTarget.category}</p>
                    <p className="font-display text-xs text-white uppercase tracking-widest">{sceneTarget.name}</p>
                  </div>
                </div>
              </div>
            );
      ...
      {loading && (
        <div className="absolute inset-0 bg-black/85 flex flex-col items-center justify-center z-40 transition-opacity duration-500">
          ...
          <p className="font-display text-xs text-white tracking-[0.3em] uppercase">Reticulating Views... 360°</p>
          <p className="text-[10px] text-neutral-stone uppercase tracking-wider mt-1">Calibrating Light and Texture</p>
        </div>
      )}
      ...
      <div className="absolute top-6 left-6 right-6 flex items-center justify-between z-30 pointer-events-none">
        {/* Info label */}
        <div className="bg-black/40 backdrop-blur-xl border border-white/10 px-4 py-2 pointer-events-auto flex items-center gap-3">
          <span className="w-2 h-2 rounded-full bg-[#775a19] animate-pulse"></span>
          <div className="text-left">
            <p className="font-label-sm text-[9px] text-white/60 uppercase tracking-widest leading-none">CURRENT SPACE</p>
            <p className="font-display text-xs font-medium text-white uppercase tracking-wider">{scene.name}</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 pointer-events-auto">
          ...
          <div className="bg-black/40 backdrop-blur-xl border border-white/10 p-2 text-white flex items-center gap-2" title="Compass Bearing">
            <Compass
          ...
          <button
            ...
            className="p-3 bg-black/40 hover:bg-white/10 backdrop-blur-xl border border-white/10 text-white rounded transition-all shadow-lg"
      ...
      {showHelp && (
        <div className="absolute inset-0 bg-black/30 pointer-events-none flex items-center justify-center z-10 transition-opacity duration-1000">
          <div className="bg-black/60 backdrop-blur-md border border-white/20 p-6 rounded text-center text-white max-w-sm flex flex-col items-center">
            <Move className="w-8 h-8 text-[#ffdea5] mb-3 animate-bounce" />
            <h4 className="font-display text-xs uppercase tracking-widest mb-1">Interactive Panorama</h4>
            <p className="text-[11px] text-white/80 leading-relaxed mb-1">
              Click and drag your cursor to pan 360°.
            </p>
            <p className="text-[10px] text-[#ffdea5] uppercase font-semibold tracking-wider">
              Click pulse points to travel
            </p>
          </div>
        </div>
      )}
      ...
      <div className="absolute left-4 top-1/2 -translate-y-1/2 z-20 flex flex-col gap-2 pointer-events-none">
        <button
          ...
          className="p-3 bg-black/55 hover:bg-primary border border-white/10 text-white shadow-2xl transition-all pointer-events-auto rounded-full"
        >
      ...
      <div className="absolute bottom-6 left-6 right-6 flex flex-col md:flex-row md:items-end justify-between gap-4 z-30 pointer-events-none">
        {/* Short info */}
        <div className="bg-black/55 backdrop-blur-xl border border-white/10 p-5 rounded max-w-md pointer-events-auto text-left">
          <span className="font-label-sm text-[9px] text-[#ffdea5] uppercase tracking-widest">{scene.category}</span>
          <p className="text-white/90 text-xs mt-1 leading-relaxed">{scene.description}</p>
        </div>

        {/* Scene Selection List (Minimal floating capsules) */}
        <div className="flex flex-wrap items-center gap-2 pointer-events-auto bg-black/30 backdrop-blur-md p-2 rounded border border-white/5 max-w-lg justify-end">
          {scenes.map((s, idx) => {
